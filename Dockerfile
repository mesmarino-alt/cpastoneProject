...
RUN apt-get update \
    && apt-get install -y --no-install-recommends build-essential default-libmysqlclient-dev libpq-dev gcc curl libssl-dev libffi-dev python3-dev \
    && rm -rf /var/lib/apt/lists/*

# default Gunicorn args (can be overridden by Railway env var)
...
# relying on an executable being present in PATH. Respect GUNICORN_CMD_ARGS so the
# worker count can be controlled via environment variables (default set above).
